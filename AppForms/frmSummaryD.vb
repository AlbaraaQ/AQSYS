...
Public Partial Class frmSummaryD
		Inherits Form
...
Private conn As SqlConnection
		...
		Public Sub New()
			AddHandler MyBase.Load, AddressOf Me.frmSummaryD_Load
			frmSummaryD.__ENCAddToList(Me)
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = frmSummaryD.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = frmSummaryD.__ENCList.Count = frmSummaryD.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = frmSummaryD.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = frmSummaryD.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								frmSummaryD.__ENCList(num) = frmSummaryD.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					frmSummaryD.__ENCList.RemoveRange(num, frmSummaryD.__ENCList.Count - num)
					frmSummaryD.__ENCList.Capacity = frmSummaryD.__ENCList.Count
				End If
				frmSummaryD.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
  ...
  Private Sub frmSummaryD_Load(ByVal sender As Object, ByVal e As EventArgs)' Handles frmSummaryD.Load
			...
			If flag Then
				Me.cmbType.Items(0) = "in receipt bonds"
				Me.cmbType.Items(1) = "in exchange bonds"
			End If
		End Sub
			...
			Try
				...
				If flag Then
					...
					If flag2 Then
						...
						frmSandQD.Show()
						frmSandQD.Navigate(Conversions.ToString(Operators.ConcatenateObject("select * from SandQD where id=", Me.dgvItems.Rows(e.RowIndex).Cells(2).Value)))
						frmSandQD.Activate()
					Else
						Dim frmSandSD As frmSandSD = New frmSandSD()
						frmSandSD.Show()
						frmSandSD.Navigate(Conversions.ToString(Operators.ConcatenateObject("select * from SandSD where id=", Me.dgvItems.Rows(e.RowIndex).Cells(2).Value)))
						frmSandSD.Activate()
					End If
				End If
			Catch ex As Exception
