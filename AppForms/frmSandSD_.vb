...
Private conn As SqlConnection
		...
		Public Sub New()
			AddHandler MyBase.Load, AddressOf Me.frmSandSD__Load
			frmSandSD_.__ENCAddToList(Me)
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = frmSandSD_.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = frmSandSD_.__ENCList.Count = frmSandSD_.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = frmSandSD_.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = frmSandSD_.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								frmSandSD_.__ENCList(num) = frmSandSD_.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					frmSandSD_.__ENCList.RemoveRange(num, frmSandSD_.__ENCList.Count - num)
					frmSandSD_.__ENCList.Capacity = frmSandSD_.__ENCList.Count
				End If
				frmSandSD_.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
  ...
  Private Sub frmSandSD__Load(ByVal sender As Object, ByVal e As EventArgs)' Handles frmSandSD_.Load
						...
						Else
							...
							Else
								...
								Else
									...
									If flag2 Then
										Me.LoadNxtNo()
										sqlCommand = New SqlCommand("insert into SandSD(id,date,emp_person,emp_id,acc_code,val,type,safe_bank_id,sales_emp,check_no,check_date,checkbank,check_state,notes,rest_id,branch,IS_Deleted)values(" + Me.txtNo.Text + ",@date,@emp_person,@emp_id,@acc_code,@val,@type,@safe_bank_id,@sales_emp,@check_no,@check_date,@checkbank,@check_state,@notes,@rest_id,@branch,@IS_Deleted)", Me.conn, sqlTransaction)
									Else
										sqlCommand = New SqlCommand("update SandSD set date=@date ,emp_person=@emp_person, emp_id=@emp_id ,acc_code=@acc_code, val=@val ,type=@type ,safe_bank_id=@safe_bank_id ,sales_emp=@sales_emp ,check_no=@check_no ,check_date=@check_date ,checkbank=@checkbank ,check_state=@check_state ,notes=@notes ,IS_Deleted=@IS_Deleted where branch=" + Conversions.ToString(MainClass.BranchNo) + " and id=" + Conversions.ToString(Me.Code), Me.conn, sqlTransaction)
									End If
									...
									flag2 = (Me.Code = -1)
									If flag2 Then
										sqlCommand = New SqlCommand("select max(id) from SandD", Me.conn, sqlTransaction)
  ...
  Private Sub txtSrchNo_TextChanged(ByVal sender As Object, ByVal e As EventArgs) Handles txtSrchNo.TextChanged
			MainClass.ISInteger(CType(e, KeyPressEventArgs))
		End Sub
		...
		Private Sub PrintRpt(type As Integer)
			...
			textObject7.Text = "يصرف سداد دفعة لحساب"
