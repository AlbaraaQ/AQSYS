...
Public Partial Class frmCostCenter
    ...
    Public Sub New()
        ...
        frmCostCenter.__ENCAddToList(Me)
        Me.conn = MainClass.ConnObj()
        Me.Code = -1
        Me.InitializeComponent()
    End Sub
    Private Shared Sub __ENCAddToList(value As Object)
        Dim _ENCList As List(Of WeakReference) = frmCostCenter.__ENCList
        Dim flag As Boolean = False
        Try
            Monitor.Enter(_ENCList, flag)
            Dim flag2 As Boolean = frmCostCenter.__ENCList.Count = frmCostCenter.__ENCList.Capacity
            If flag2 Then
                Dim num As Integer = 0
                Dim num2 As Integer = 0
                Dim num3 As Integer = frmCostCenter.__ENCList.Count - 1
                ...
                While True
                    ...
                    If num5 > num6 Then
                        Exit While
                    End If
                    Dim weakReference As WeakReference = frmCostCenter.__ENCList(num4)
                    flag2 = weakReference.IsAlive
                    If flag2 Then
                        Dim flag3 As Boolean = num4 <> num
                        If flag3 Then
                            frmCostCenter.__ENCList(num) = frmCostCenter.__ENCList(num4)
                        End If
                        num += 1
                    End If
                    num4 += 1
                End While
                frmCostCenter.__ENCList.RemoveRange(num, frmCostCenter.__ENCList.Count - num)
                frmCostCenter.__ENCList.Capacity = frmCostCenter.__ENCList.Count
            End If
            frmCostCenter.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
        Finally
    ...
    Private Sub LoadDG()
        ...
        Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select * from costcenter", Me.conn)
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        Try
            ...
            If flag Then
                Interaction.MsgBox("أدخل الاسم", MsgBoxStyle.OkOnly, Nothing)
                Me.txtName.Focus()
            Else
                ...
                flag = (Me.Code = -1)
                If flag Then
                    Dim sqlCommand As SqlCommand = New SqlCommand("insert into costcenter(name) values('" + Me.txtName.Text + "')", Me.conn)
                    sqlCommand.ExecuteNonQuery()
                    Me.txtName.Text = ""
                    Me.txtName.Focus()
                Else
                    Dim sqlCommand2 As SqlCommand = New SqlCommand("update costcenter set  name ='" + Me.txtName.Text + "' where id=" + Conversions.ToString(Me.Code), Me.conn)
        ...
        Try
            ...
            If flag Then
                ...
                If flag2 Then
                    Me.conn.Open()
                End If
                Dim sqlCommand As SqlCommand = New SqlCommand("delete from costcenter where id=" + Conversions.ToString(Me.Code), Me.conn)
                sqlCommand.ExecuteNonQuery()
                MessageBox.Show("تم الحذف", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Me.LoadDG()
                Me.CLR()
            Else
                MessageBox.Show("اختر مركز تكلفة ليتم حذفه", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
            End If
        Catch ex As Exception
    ...
    Private Sub btnLast_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLast.Click
        Me.Navigate("select top 1 * from costcenter order by id desc")
    End Sub

    Private Sub btnFirst_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnFirst.Click
        Me.Navigate("select top 1 * from costcenter order by id asc")
    End Sub

    Private Sub btnNext_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnNext.Click
        Me.Navigate("select top 1 * from costcenter where id>" + Conversions.ToString(Me.Code) + " order by id asc")
    End Sub

    Private Sub btnPrevious_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrevious.Click
        Me.Navigate("select top 1 * from costcenter where id<" + Conversions.ToString(Me.Code) + " order by id desc")
    End Sub
    ...
    Private Sub txtName_KeyDown_1(ByVal sender As Object, ByVal e As KeyEventArgs) Handles txtName.KeyDown
