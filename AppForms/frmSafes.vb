...
Private conn As SqlConnection
		...
		Public Sub New()
			AddHandler MyBase.Load, AddressOf Me.frmSafes_Load
			frmSafes.__ENCAddToList(Me)
			Me.conn = MainClass.ConnObj()
			Me.Code = -1
			Me.InitializeComponent()
		End Sub
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = frmSafes.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = frmSafes.__ENCList.Count = frmSafes.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = frmSafes.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = frmSafes.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								frmSafes.__ENCList(num) = frmSafes.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					frmSafes.__ENCList.RemoveRange(num, frmSafes.__ENCList.Count - num)
					frmSafes.__ENCList.Capacity = frmSafes.__ENCList.Count
				End If
				frmSafes.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		Private Sub LoadDG()
			Me.dgvSafes.Rows.Clear()
			Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select Safes.id as Safe_id,Safes.name as safe,Branches.name as branch from Safes,Branches  where Safes.IS_Deleted=0 and Safes.branch=branches.id", Me.conn)
			...
			While True
				...
				If num4 > num5 Then
					Exit While
				End If
				Me.dgvSafes.Rows.Add()
				Me.dgvSafes.Rows(num3).Cells(0).Value = RuntimeHelpers.GetObjectValue(dataTable.Rows(num3)("Safe_id"))
				Me.dgvSafes.Rows(num3).Cells(1).Value = RuntimeHelpers.GetObjectValue(dataTable.Rows(num3)("safe"))
				Me.dgvSafes.Rows(num3).Cells(2).Value = RuntimeHelpers.GetObjectValue(dataTable.Rows(num3)("branch"))
				num3 += 1
			End While
			Me.dgvSafes.ClearSelection()
		End Sub

  Private Sub frmSafes_Load(ByVal sender As Object, ByVal e As EventArgs)' Handles frmSafes.Load
			Me.LoadBranches()
			Me.LoadStates()
			Me.LoadEmps()
			Me.LoadDG()
			...
			If flag Then
				Me.cmbStatus.Items(0) = "active"
				Me.cmbStatus.Items(1) = "closed"
			End If
		End Sub
  ...
  Private Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
			...
			Try
				...
				If flag Then
					...
					If flag2 Then
						MessageBox.Show("ادخل اسم المخزن", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
					Else
						MessageBox.Show("Enter store name", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
					End If
					Me.txtName.Focus()
				Else
					...
					Else
						...
						If flag2 Then
							...
							Else
								MessageBox.Show("you must select responsible employee", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
							End If
						...
						Else
							...
							Else
								MessageBox.Show("you not select responsible employee", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
							End If
							...
							Else
								MessageBox.Show("you repeat inserting employee", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
							End If
							...
							flag2 = (Me.Code <> -1)
							If flag2 Then
								sqlCommand = New SqlCommand("delete from Safe_Emps where safe_id=" + Conversions.ToString(Me.Code), Me.conn)
								sqlCommand.ExecuteNonQuery()
							End If
							flag2 = (Me.Code = -1)
							If flag2 Then
								sqlCommand = New SqlCommand("insert into Safes(name,branch,status,IS_Default,notes,IS_Deleted)values(@name,@branch,@status,@IS_Default,@notes,@IS_Deleted)", Me.conn)
							Else
								sqlCommand = New SqlCommand("update Safes set name=@name,branch=@branch,status=@status,IS_Default=@IS_Default,notes=@notes,IS_Deleted=@IS_Deleted where id=" + Conversions.ToString(Me.Code), Me.conn)
							End If
							...
							If flag2 Then
								num14 = Conversions.ToInteger(Me.cmbBranches.SelectedValue)
							End If
							sqlCommand.Parameters.Add("@branch", SqlDbType.Int).Value = num14
							Dim num15 As Integer = -1
							flag2 = (Me.cmbStatus.SelectedValue IsNot Nothing)
							If flag2 Then
								num15 = Conversions.ToInteger(Me.cmbStatus.SelectedValue)
							End If
							sqlCommand.Parameters.Add("@status", SqlDbType.Int).Value = num15
							Dim num16 As Integer = 0
							flag2 = Me.chkDefault.Checked
							If flag2 Then
								num16 = 1
							End If
							sqlCommand.Parameters.Add("@IS_Default", SqlDbType.Bit).Value = num16
							...
							While True
								...
								If num20 > num5 Then
									Exit While
								End If
								sqlCommand = New SqlCommand(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("insert into Safe_Emps(safe_id,emp_id)values(" + Conversions.ToString(Me.Code) + ",", Me.dgvSafeEmps.Rows(num19).Cells(0).Value), ")")), Me.conn)
								sqlCommand.ExecuteNonQuery()
								num19 += 1
							End While
							Me.LoadDG()
  ...
  Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
			Try
				...
				If flag Then
					Try
						Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select count(id) from inv where is_deleted=0 and safe=" + Conversions.ToString(Me.Code), Me.conn)
						Dim dataTable As DataTable = New DataTable()
						sqlDataAdapter.Fill(dataTable)
						flag = (Conversion.Val(Operators.ConcatenateObject("", dataTable.Rows(0)(0))) > 0.0)
						If flag Then
							Interaction.MsgBox("هذا المخزن مرتبط بفواتير ولا يمكن حذفه", MsgBoxStyle.OkOnly, Nothing)
							Return
						End If
					Catch ex As Exception
					End Try
					...
					If flag Then
						Me.conn.Open()
					End If
					Dim sqlCommand As SqlCommand = New SqlCommand("update Safes set IS_Deleted=1 where id=" + Conversions.ToString(Me.Code), Me.conn)
					sqlCommand.ExecuteNonQuery()
					sqlCommand = New SqlCommand("delete from Safe_Emps where safe_id=" + Conversions.ToString(Me.Code), Me.conn)
					sqlCommand.ExecuteNonQuery()
					flag = (Operators.CompareString(MainClass.Language, "ar", False) = 0)
				...
				Else
					flag = (Operators.CompareString(MainClass.Language, "ar", False) = 0)
					If flag Then
						MessageBox.Show("اختر مخزن ليتم حذفه", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
					Else
						MessageBox.Show("choose store to be deleted", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
					End If
				End If
		...
		Private Sub dgvRowChng(row_index As Integer)
			...
			Me.Code = CInt(Math.Round(Conversion.Val(Operators.ConcatenateObject("", Me.dgvSafes.Rows(row_index).Cells(0).Value))))
			Me.Navigate("select * from Safes where id=" + Conversions.ToString(Me.Code))
		End Sub

  Private Sub dgvSafes_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dgvSafes.CellClick
  ...
  Private Sub btnClose_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnClose.Click
		...
		Private Sub ReadData(dr As SqlDataReader)
			...
			If flag Then
				...
				dr.Close()
				Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select * from Safe_Emps where safe_id=" + Conversions.ToString(Me.Code), Me.conn)
				Dim dataTable As DataTable = New DataTable()
				sqlDataAdapter.Fill(dataTable)
				Dim num As Integer = 0
				Dim num2 As Integer = dataTable.Rows.Count - 1
  ...
  Private Sub btnLast_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLast.Click
			Me.Navigate("select top 1 * from Safes where IS_Deleted=0 order by id desc")
		End Sub

  Private Sub btnNext_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnNext.Click
			Me.Navigate("select top 1 * from Safes where IS_Deleted=0 and id>" + Conversions.ToString(Me.Code) + " order by id asc")
		End Sub

  Private Sub btnFirst_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnFirst.Click
			Me.Navigate("select top 1 * from Safes where IS_Deleted=0 order by id asc")
		End Sub

  Private Sub btnPrevious_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrevious.Click
			Me.Navigate("select top 1 * from Safes where IS_Deleted=0 and id<" + Conversions.ToString(Me.Code) + " order by id desc")
		End Sub
  ...
  Private Sub GroupBox3_Enter(ByVal sender As Object, ByVal e As EventArgs) Handles GroupBox3.Enter
		...
		Private Sub PrintRpt(type As Integer)
			Dim flag As Boolean = Me.dgvSafes.Rows.Count = 0
			If flag Then
			...
			Else
				...
				Dim num As Integer = 0
				Dim num2 As Integer = Me.dgvSafes.Rows.Count - 1
				Dim num3 As Integer = num
				While True
					Dim num4 As Integer = num3
					Dim num5 As Integer = num2
					If num4 > num5 Then
						Exit While
					End If
					dataTable.Rows.Add(New Object() { RuntimeHelpers.GetObjectValue(Me.dgvSafes.Rows(num3).Cells(1).Value), RuntimeHelpers.GetObjectValue(Me.dgvSafes.Rows(num3).Cells(2).Value) })
					num3 += 1
				End While
  ...
  Private Sub cmbBranches_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbBranches.SelectedIndexChanged
		End Sub
  ...
  Private Sub dgvSafeEmps_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dgvSafeEmps.CellClick
			Try
				...
				If flag Then
					Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter(Conversions.ToString(Operators.ConcatenateObject("select count(id) from inv where is_deleted=0 and safe=" + Conversions.ToString(Me.Code) + " and sales_emp=", Me.dgvSafeEmps.Rows(e.RowIndex).Cells(0).Value)), Me.conn)
					Dim dataTable As DataTable = New DataTable()
					sqlDataAdapter.Fill(dataTable)
					flag = (Conversion.Val(Operators.ConcatenateObject("", dataTable.Rows(0)(0))) > 0.0)
					If flag Then
						Interaction.MsgBox("هذا الموظف مرتبط بفواتير لهذا المخزن ولا يمكن حذفه", MsgBoxStyle.OkOnly, Nothing)
					Else
