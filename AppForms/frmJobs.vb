...
Public Partial Class frmJobs
    ...
    Public Sub New()
        AddHandler MyBase.Load, AddressOf Me.frmJobs_Load
        frmJobs.__ENCAddToList(Me)
        Me.conn = MainClass.ConnObj()
        Me.Code = -1
        Me.InitializeComponent()
    End Sub
    Private Shared Sub __ENCAddToList(value As Object)
        Dim _ENCList As List(Of WeakReference) = frmJobs.__ENCList
        Dim flag As Boolean = False
        Try
            Monitor.Enter(_ENCList, flag)
            Dim flag2 As Boolean = frmJobs.__ENCList.Count = frmJobs.__ENCList.Capacity
            If flag2 Then
                Dim num As Integer = 0
                Dim num2 As Integer = 0
                Dim num3 As Integer = frmJobs.__ENCList.Count - 1
                ...
                While True
                    ...
                    If num5 > num6 Then
                        Exit While
                    End If
                    Dim weakReference As WeakReference = frmJobs.__ENCList(num4)
                    flag2 = weakReference.IsAlive
                    If flag2 Then
                        Dim flag3 As Boolean = num4 <> num
                        If flag3 Then
                            frmJobs.__ENCList(num) = frmJobs.__ENCList(num4)
                        End If
                        num += 1
                    End If
                    num4 += 1
                End While
                frmJobs.__ENCList.RemoveRange(num, frmJobs.__ENCList.Count - num)
                frmJobs.__ENCList.Capacity = frmJobs.__ENCList.Count
            End If
            frmJobs.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
        Finally
    ...
    Private Sub LoadDG()
        ...
        Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select * from jobs", Me.conn)
        ...
        While True
            ...
            End If
            Me.dgvJobs.Rows.Add()
            Me.dgvJobs.Rows(num3).Cells(0).Value = RuntimeHelpers.GetObjectValue(dataTable.Rows(num3)("id"))
            Me.dgvJobs.Rows(num3).Cells(1).Value = RuntimeHelpers.GetObjectValue(dataTable.Rows(num3)("name"))
            num3 += 1
        End While
        Me.dgvJobs.ClearSelection()
    End Sub

    Private Sub frmJobs_Load(ByVal sender As Object, ByVal e As EventArgs) 'Handles frmJobs.Load
        Me.LoadDG()
    ...
    Private Sub btnSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnSave.Click
        Try
            ...
            If flag Then
                ...
                flag2 = (Me.Code = -1)
                If flag2 Then
                    Dim sqlCommand As SqlCommand = New SqlCommand("insert into jobs(name) values('" + Me.txtName.Text + "')", Me.conn)
                    sqlCommand.ExecuteNonQuery()
                    Me.txtName.Text = ""
                    Me.txtName.Focus()
                Else
                    Dim sqlCommand2 As SqlCommand = New SqlCommand("update jobs set  name ='" + Me.txtName.Text + "' where id=" + Conversions.ToString(Me.Code), Me.conn)
                    sqlCommand2.ExecuteNonQuery()
                    Me.txtName.Focus()
                End If
                Me.LoadDG()
            Else
                MessageBox.Show("ادخل الوظيفة", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Me.txtName.Focus()
            End If
    ...
    Private Sub dgvJobs_RowChng(row_index As Integer)
        Me.txtName.Text = Me.dgvJobs.Rows(row_index).Cells(1).Value.ToString()
        Me.Code = CInt(Math.Round(Conversion.Val(Operators.ConcatenateObject("", Me.dgvJobs.Rows(row_index).Cells(0).Value))))
    End Sub

    Private Sub dgvJobs_CellClick(ByVal sender As Object, ByVal e As DataGridViewCellEventArgs) Handles dgvJobs.CellClick
        Dim flag As Boolean = e.RowIndex >= 0
        If flag Then
            Me.dgvJobs_RowChng(e.RowIndex)
        End If
    End Sub
        ...
        Try
            ...
            If flag Then
                ...
                If flag2 Then
                    Me.conn.Open()
                End If
                Dim sqlCommand As SqlCommand = New SqlCommand("delete from jobs where id=" + Conversions.ToString(Me.Code), Me.conn)
                sqlCommand.ExecuteNonQuery()
                MessageBox.Show("تم الحذف", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
                Me.LoadDG()
                Me.CLR()
            Else
                MessageBox.Show("اختر وظيفة ليتم حذفها", "", MessageBoxButtons.OK, MessageBoxIcon.Asterisk)
            End If
        Catch ex As Exception
    ...
    Private Sub btnLast_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLast.Click
        Me.Navigate("select top 1 * from jobs order by id desc")
    End Sub

    Private Sub btnFirst_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnFirst.Click
        Me.Navigate("select top 1 * from jobs order by id asc")
    End Sub

    Private Sub btnNext_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnNext.Click
        Me.Navigate("select top 1 * from jobs where id>" + Conversions.ToString(Me.Code) + " order by id asc")
    End Sub

    Private Sub btnPrevious_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrevious.Click
        Me.Navigate("select top 1 * from jobs where id<" + Conversions.ToString(Me.Code) + " order by id desc")
    End Sub

    Private Sub PrintRpt(type As Integer)
        Dim flag As Boolean = Me.dgvJobs.Rows.Count = 0
        If flag Then
        ...
        Else
            ...
            Dim num As Integer = 0
            Dim num2 As Integer = Me.dgvJobs.Rows.Count - 1
            Dim num3 As Integer = num
            While True
                Dim num4 As Integer = num3
                Dim num5 As Integer = num2
                If num4 > num5 Then
                    Exit While
                End If
                dataTable.Rows.Add(New Object() {RuntimeHelpers.GetObjectValue(Me.dgvJobs.Rows(num3).Cells(1).Value)})
                num3 += 1
            End While
