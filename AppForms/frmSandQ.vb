...
Private conn As SqlConnection
		...
		Public Sub New()
			AddHandler MyBase.Load, AddressOf Me.frmSandQ_Load
			frmSandQ.__ENCAddToList(Me)
		...
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = frmSandQ.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = frmSandQ.__ENCList.Count = frmSandQ.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = frmSandQ.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = frmSandQ.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								frmSandQ.__ENCList(num) = frmSandQ.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					frmSandQ.__ENCList.RemoveRange(num, frmSandQ.__ENCList.Count - num)
					frmSandQ.__ENCList.Capacity = frmSandQ.__ENCList.Count
				End If
				frmSandQ.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		Private Sub LoadNxtNo()
			Try
				...
				Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select max(id) from SandQ where branch=" + Conversions.ToString(MainClass.BranchNo), Me.conn)
		...
		Private Sub LoadDG(cond As String)
			...
			Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter("select SandQ.id,SandQ.date,SandQ.val,Employees.name from SandQ,Employees  where SandQ.IS_Deleted=0 and " + cond + " SandQ.sales_emp=Employees.id", Me.conn)
  ...
  Private Sub frmSandQ_Load(ByVal sender As Object, ByVal e As EventArgs)' Handles frmSandQ.Load
		...
		Public Function InsertAuto(_val As Double, _cust As Integer, _custname As String, _khazna As Integer, _khznaname As String, _date As DateTime) As Object
			...
			Try
				...
				Catch ex As Exception
				End Try
				sqlCommand = New SqlCommand("insert into SandQ(id,date,cust_id,val,type,safe_bank_id,sales_emp,check_no,check_date,checkbank,check_state,rest_id,notes,branch,IS_Deleted)values(" + Conversions.ToString(value) + ",@date,@cust_id,@val,@type,@safe_bank_id,@sales_emp,@check_no,@check_date,@checkbank,@check_state,@rest_id,@notes,@branch,@IS_Deleted)", Me.conn, sqlTransaction)
					...
					Else
						...
						Else
							...
							Else
								...
								If flag2 Then
									Me.LoadNxtNo()
									sqlCommand = New SqlCommand("insert into SandQ(id,date,cust_id,val,type,safe_bank_id,sales_emp,check_no,check_date,checkbank,check_state,rest_id,notes,branch,ref_no,IS_Deleted)values(" + Me.txtNo.Text + ",@date,@cust_id,@val,@type,@safe_bank_id,@sales_emp,@check_no,@check_date,@checkbank,@check_state,@rest_id,@notes,@branch,@ref_no,@IS_Deleted)", Me.conn, sqlTransaction)
								Else
									sqlCommand = New SqlCommand("update SandQ set date=@date ,cust_id=@cust_id ,val=@val ,type=@type ,safe_bank_id=@safe_bank_id ,sales_emp=@sales_emp ,check_no=@check_no ,check_date=@check_date ,checkbank=@checkbank ,check_state=@check_state ,notes=@notes, ref_no=@ref_no ,IS_Deleted=@IS_Deleted where branch=" + Conversions.ToString(MainClass.BranchNo) + " and id=" + Conversions.ToString(Me.Code), Me.conn, sqlTransaction)
								End If
								...
								sqlCommand.Parameters.Add("@notes", SqlDbType.NVarChar).Value = Me.txtNotes.Text
								sqlCommand.Parameters.Add("@branch", SqlDbType.Int).Value = MainClass.BranchNo
								sqlCommand.Parameters.Add("@ref_no", SqlDbType.NVarChar).Value = Me.txtRefNo.Text
								sqlCommand.Parameters.Add("@IS_Deleted", SqlDbType.Bit).Value = 0
								sqlCommand.ExecuteNonQuery()
								flag2 = (Me.Code = -1)
								If flag2 Then
								...
								End If
								Dim value As String = "سند قبض خارجي رقم: " + Conversions.ToString(Me.Code) + " خاصة العميل:" + Me.cmbClients.Text
								flag2 = (Operators.CompareString(Me.txtRefNo.Text.Trim(), "", False) <> 0)
								If flag2 Then
									value = "سند قبض خارجي رقم: " + Me.txtRefNo.Text.Trim() + " خاصة العميل:" + Me.cmbClients.Text
								End If
								sqlCommand = New SqlCommand("insert into Restrictions(id,date,doc_no,type,state,notes,branch,IS_Deleted)values(@id,@date,@doc_no,@type,@state,@notes,@branch,@IS_Deleted)", Me.conn, sqlTransaction)
								sqlCommand.Parameters.Add("@id", SqlDbType.Int).Value = num
								sqlCommand.Parameters.Add("@date", SqlDbType.DateTime).Value = Me.txtDate.Value
								...
								sqlCommand.Parameters.Add("@type", SqlDbType.Int).Value = 5
								sqlCommand.Parameters.Add("@state", SqlDbType.Int).Value = 1
								sqlCommand.Parameters.Add("@notes", SqlDbType.NVarChar).Value = value
								sqlCommand.Parameters.Add("@branch", SqlDbType.Int).Value = MainClass.BranchNo
								sqlCommand.Parameters.Add("@IS_Deleted", SqlDbType.Bit).Value = 0
								sqlCommand.ExecuteNonQuery()
								sqlCommand = New SqlCommand("insert into Restrictions_Sub(res_id,dept,credit,acc_no,notes,branch)values(@res_id,@dept,@credit,@acc_no,@notes,@branch)", Me.conn, sqlTransaction)
  ...
  Private Sub btnDelete_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnDelete.Click
			Try
				...
				If flag Then
					...
					If flag2 Then
						Me.conn.Open()
					End If
					Dim sqlCommand As SqlCommand = New SqlCommand("update SandQ set IS_Deleted=1 where branch=" + Conversions.ToString(MainClass.BranchNo) + " and id=" + Conversions.ToString(Me.Code), Me.conn)
					sqlCommand.ExecuteNonQuery()
		...
		Private Sub dgvRowChng(row_index As Integer)
			...
			Me.Navigate("select * from SandQ where id=" + Conversions.ToString(Me.Code))
  ...
  Private Sub btnClose_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnClose.Click
		...
		Private Sub ReadData(dr As SqlDataReader)
			...
			If flag Then
				...
				Me.txtNotes.Text = Conversions.ToString(Operators.ConcatenateObject("", dr("notes")))
				Try
					Me.txtRefNo.Text = Conversions.ToString(Operators.ConcatenateObject("", dr("ref_no")))
				Catch ex As Exception
				End Try
				dr.Close()
			End If
  ...
  Private Sub btnLast_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnLast.Click
			Me.Navigate("select top 1 * from SandQ where " + Me.GetCondBranch() + " IS_Deleted=0 order by id desc")
		End Sub

  Private Sub btnNext_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnNext.Click
			Me.Navigate(String.Concat(New String() { "select top 1 * from SandQ where ", Me.GetCondBranch(), " IS_Deleted=0 and id>", Conversions.ToString(Me.Code), " order by id asc" }))
		End Sub

  Private Sub btnFirst_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnFirst.Click
			Me.Navigate("select top 1 * from SandQ where " + Me.GetCondBranch() + " IS_Deleted=0 order by id asc")
		End Sub

  Private Sub btnPrevious_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnPrevious.Click
			Me.Navigate(String.Concat(New String() { "select top 1 * from SandQ where ", Me.GetCondBranch(), " IS_Deleted=0 and id<", Conversions.ToString(Me.Code), " order by id desc" }))
		End Sub
		...
		Private Sub Search()
			...
			Dim flag As Boolean = MainClass.BranchNo <> -1
			If flag Then
				str = "SandQ.branch=" + Conversions.ToString(MainClass.BranchNo) + " and "
			End If
			flag = (Operators.CompareString(Me.txtSrchNo.Text.Trim(), "", False) <> 0)
			Dim cond As String
			If flag Then
				cond = str + " SandQ.id=" + Me.txtSrchNo.Text + " and "
			Else
  ...
  Private Sub cmbClients_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbClients.SelectedIndexChanged
		...
		Private Sub PrintRpt(type As Integer)
			...
			textObject5.Text = "استلمت من السيد"
			...
			textObject6.Text = Me.cmbClients.Text
			...
			textObject12.Text = Me.cmbClients.Text
			...
			If flag Then
				...
				Dim textObject17 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 2 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "txtTel" }, Nothing, Nothing, Nothing), TextObject)
				...
				Dim textObject19 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 2 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "txtFax" }, Nothing, Nothing, Nothing), TextObject)
				textObject19.Text = Conversions.ToString(Operators.ConcatenateObject("", dataTable.Rows(0)("Fax")))
			End If
			Try
				flag = Convert.ToBoolean(RuntimeHelpers.GetObjectValue(dataTable.Rows(0)("is_sandQ_stamp")))
				If flag Then
					Dim textObject20 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "lblstamp" }, Nothing, Nothing, Nothing), TextObject)
					textObject20.Text = "الختم"
					Dim dataTable2 As DataTable = New DataTable()
					sqlDataAdapter.Fill(dataTable2)
					dataTable2.Rows(0)("Logo") = CType(dataTable.Rows(0)("stamp"), Byte())
					Dim instance2 As Object = NewLateBinding.LateGet(obj, Nothing, "Subreports", New Object() { "rptStamp" }, Nothing, Nothing, Nothing)
			...
			End Try
			Try
				flag = Convert.ToBoolean(RuntimeHelpers.GetObjectValue(dataTable.Rows(0)("is_sandQ_sign")))
				If flag Then
					Dim textObject21 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "lblsign" }, Nothing, Nothing, Nothing), TextObject)
					textObject21.Text = "التوقيع"
					Dim dataTable3 As DataTable = New DataTable()
					sqlDataAdapter.Fill(dataTable3)
					dataTable3.Rows(0)("Logo") = CType(dataTable.Rows(0)("sandQ_sign"), Byte())
					Dim instance3 As Object = NewLateBinding.LateGet(obj, Nothing, "Subreports", New Object() { "rptSign" }, Nothing, Nothing, Nothing)
					...
					End If
				Else
					Dim subreportObject3 As SubreportObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "Subreport3" }, Nothing, Nothing, Nothing), SubreportObject)
					subreportObject3.ObjectFormat.EnableSuppress = True
				End If
			Catch ex3 As Exception
				Dim subreportObject4 As SubreportObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "Subreport3" }, Nothing, Nothing, Nothing), SubreportObject)
				subreportObject4.ObjectFormat.EnableSuppress = True
			End Try
			Try
				flag = Not Convert.ToBoolean(RuntimeHelpers.GetObjectValue(dataTable.Rows(0)("print_sanad_header")))
				If flag Then
					Dim subreportObject5 As SubreportObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "Subreport1" }, Nothing, Nothing, Nothing), SubreportObject)
					subreportObject5.ObjectFormat.EnableSuppress = True
				End If
			...
			Else
				Try
					...
					Dim array4 As Object() = New Object() { 1, False, 1, num }
					...
					If array2(3) Then
						num = CInt(Conversions.ChangeType(RuntimeHelpers.GetObjectValue(array4(3)), GetType(Integer)))
					End If
  ...
  Private Sub Button11_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button11.Click
			Try
			'New frmSrchClient() With { .Form_Type = 10, .frm10 = Me }.ShowDialog()
