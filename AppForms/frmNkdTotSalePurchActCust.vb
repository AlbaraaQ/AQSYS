...
Private conn As SqlConnection
		...
		Public Sub New()
			AddHandler MyBase.Load, AddressOf Me.frmNkdTotSalePurchActCust_Load
			frmNkdTotSalePurchActCust.__ENCAddToList(Me)
			Me.conn = MainClass.ConnObj()
			Me.ClientID = -1
			Me.InitializeComponent()
		End Sub
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = frmNkdTotSalePurchActCust.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = frmNkdTotSalePurchActCust.__ENCList.Count = frmNkdTotSalePurchActCust.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = frmNkdTotSalePurchActCust.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = frmNkdTotSalePurchActCust.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								frmNkdTotSalePurchActCust.__ENCList(num) = frmNkdTotSalePurchActCust.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					frmNkdTotSalePurchActCust.__ENCList.RemoveRange(num, frmNkdTotSalePurchActCust.__ENCList.Count - num)
					frmNkdTotSalePurchActCust.__ENCList.Capacity = frmNkdTotSalePurchActCust.__ENCList.Count
				End If
				frmNkdTotSalePurchActCust.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		Private Sub CalcStock()
			...
			Try
				...
				If flag Then
					right = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("acts.id=", Me.cmbActs.SelectedValue), " and "))
				End If
				Dim right2 As String = ""
				flag = Not Me.chkAllCust.Checked
				If flag Then
					right2 = Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("customers.id=", Me.cmbCust.SelectedValue), " and "))
				End If
				Dim str As String = ""
				...
				While True
					...
					Dim sqlDataAdapter2 As SqlDataAdapter = New SqlDataAdapter(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject("select sum(val),sum(val1*exchange_price + taxval - discount) from inv,inv_sub,customers,acts where " + str + " Inv_Sub.currency_from=", dataTable.Rows(num5)("id")), " and inv.cust_id=customers.id and customers.act=acts.id and "), right), right2), " date>=@date1 and date<=@date2 and inv.proc_type=1 "), " and inv_sub.proc_type=1 and inv.proc_id=inv_sub.proc_id  and inv.IS_Deleted=0")), Me.conn)
					...
					End If
					sqlDataAdapter2 = New SqlDataAdapter(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject("select sum(val),sum(val1*exchange_price + taxval - discount) from inv,inv_sub,customers,acts where " + str + " Inv_Sub.currency_from=", dataTable.Rows(num5)("id")), " and inv.cust_id=customers.id and customers.act=acts.id and "), right), right2), " date>=@date1 and date<=@date2 and inv.proc_type=2 "), " and inv_sub.proc_type=1 and inv.proc_id=inv_sub.proc_id  and inv.IS_Deleted=0")), Me.conn)
					sqlDataAdapter2.SelectCommand.Parameters.Add("@date1", SqlDbType.DateTime).Value = Me.txtDateFrom.Value.ToShortDateString()
					sqlDataAdapter2.SelectCommand.Parameters.Add("@date2", SqlDbType.DateTime).Value = dateTime
					...
					End If
					sqlDataAdapter2 = New SqlDataAdapter(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject(Operators.ConcatenateObject("select sum(val),sum(val1*exchange_price + taxval - discount) from inv,inv_sub,customers,acts where " + str + " Inv_Sub.currency_from=", dataTable.Rows(num5)("id")), " and inv.cust_id=customers.id and customers.act=acts.id and "), right), right2), " date>=@date1 and date<=@date2 and inv.proc_type=2 "), " and inv_sub.proc_type=2 and inv.proc_id=inv_sub.proc_id  and inv.IS_Deleted=0")), Me.conn)
					sqlDataAdapter2.SelectCommand.Parameters.Add("@date1", SqlDbType.DateTime).Value = Me.txtDateFrom.Value.ToShortDateString()
					sqlDataAdapter2.SelectCommand.Parameters.Add("@date2", SqlDbType.DateTime).Value = dateTime
					dataTable2 = New DataTable()
					sqlDataAdapter2.Fill(dataTable2)
  ...
  Private Sub btnShow_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnShow.Click
			...
			If flag Then
				...
				Me.cmbActs.Focus()
			Else
				flag = (Me.chkAllCust.Enabled And Not Me.chkAllCust.Checked And Me.cmbCust.SelectedIndex = -1)
				If flag Then
  ...
  Private Sub btnClose_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnClose.Click
		...
		Private Sub PrintRpt(type As Integer)
			...
			Else
				...
				Dim textObject2 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "lblact" }, Nothing, Nothing, Nothing), TextObject)
				textObject2.Text = "جهة العمل"
				Dim textObject3 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "lblcust" }, Nothing, Nothing, Nothing), TextObject)
				textObject3.Text = "العميل"
				...
				End If
				Dim textObject4 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "act" }, Nothing, Nothing, Nothing), TextObject)
				textObject4.Text = text2
				Dim textObject5 As TextObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "cust" }, Nothing, Nothing, Nothing), TextObject)
				textObject5.Text = text3
				Dim boxObject As BoxObject = CType(NewLateBinding.LateGet(NewLateBinding.LateGet(NewLateBinding.LateGet(obj, Nothing, "ReportDefinition", New Object(-1) {}, Nothing, Nothing, Nothing), Nothing, "Sections", New Object() { 1 }, Nothing, Nothing, Nothing), Nothing, "ReportObjects", New Object() { "box6" }, Nothing, Nothing, Nothing), BoxObject)
				...
				textObject8.Text = Me.txtSumPurch.Text
  ...
  Private Sub frmNkdTotSalePurchActCust_Load(ByVal sender As Object, ByVal e As EventArgs)' Handles frmNkdTotSalePurchActCust.Load
  ...
  Private Sub chkAllActs_CheckedChanged(ByVal sender As Object, ByVal e As EventArgs) Handles chkAllActs.CheckedChanged
			...
			If checked Then
				...
				Me.chkAllCust.Enabled = False
			Else
				Me.cmbActs.Enabled = True
				Me.cmbCust.Enabled = True
				Me.chkAllCust.Checked = False
				Me.chkAllCust.Enabled = True
			End If
		End Sub
  ...
  Private Sub cmbActs_SelectedIndexChanged(ByVal sender As Object, ByVal e As EventArgs) Handles cmbActs.SelectedIndexChanged
			Try
				Dim sqlDataAdapter As SqlDataAdapter = New SqlDataAdapter(Conversions.ToString(Operators.ConcatenateObject(Operators.ConcatenateObject("select id,name from customers where act=", Me.cmbActs.SelectedValue), " order by id")), Me.conn)
				Dim dataTable As DataTable = New DataTable()
				sqlDataAdapter.Fill(dataTable)
  ...
  Private Sub Button11_Click(ByVal sender As Object, ByVal e As EventArgs) Handles Button11.Click
			Try
			'New frmSrchClient() With { .Form_Type = 8, .frm8 = Me }.ShowDialog()
			Using frm As New frmSrchClient()
				frm.Form_Type = 8
				frm.frm8 = Me
				frm.ShowDialog()
			End Using
			Me.cmbCust.SelectedValue = Me.ClientID
			Catch ex As Exception
			End Try
		End Sub
	End Class
