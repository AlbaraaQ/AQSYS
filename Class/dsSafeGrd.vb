...
Imports System.Xml.Serialization
	<HelpKeyword("vs.data.DataSet"), XmlRoot("dsSafeGrd"), XmlSchemaProvider("GetTypedDataSetSchema"), DesignerCategory("code"), ToolboxItem(True), Serializable()>
	Public Class dsSafeGrd
		Inherits DataSet

		Private Shared __ENCList As List(Of WeakReference) = New List(Of WeakReference)()

		Private tableDataTable1 As dsSafeGrd.DataTable1DataTable

		Private _schemaSerializationMode As SchemaSerializationMode
		Shared Sub New()
		End Sub
		Private Shared Sub __ENCAddToList(value As Object)
			Dim _ENCList As List(Of WeakReference) = dsSafeGrd.__ENCList
			Dim flag As Boolean = False
			Try
				Monitor.Enter(_ENCList, flag)
				Dim flag2 As Boolean = dsSafeGrd.__ENCList.Count = dsSafeGrd.__ENCList.Capacity
				If flag2 Then
					Dim num As Integer = 0
					Dim num2 As Integer = 0
					Dim num3 As Integer = dsSafeGrd.__ENCList.Count - 1
					...
					While True
						...
						If num5 > num6 Then
							Exit While
						End If
						Dim weakReference As WeakReference = dsSafeGrd.__ENCList(num4)
						flag2 = weakReference.IsAlive
						If flag2 Then
							Dim flag3 As Boolean = num4 <> num
							If flag3 Then
								dsSafeGrd.__ENCList(num) = dsSafeGrd.__ENCList(num4)
							End If
							num += 1
						End If
						num4 += 1
					End While
					dsSafeGrd.__ENCList.RemoveRange(num, dsSafeGrd.__ENCList.Count - num)
					dsSafeGrd.__ENCList.Capacity = dsSafeGrd.__ENCList.Count
				End If
				dsSafeGrd.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
			Finally
		...
		Protected Sub New(info As SerializationInfo, context As StreamingContext)
			...
			Else
				...
				If flag Then
					...
					If flag Then
						MyBase.Tables.Add(New dsSafeGrd.DataTable1DataTable(dataSet.Tables("DataTable1")))
					End If
		...
		Public ReadOnly Property DataTable1 As dsSafeGrd.DataTable1DataTable
			Get
				Return Me.tableDataTable1
			End Get
		End Property

		<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), Browsable(True), DebuggerNonUserCode(), DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)>
		...
		Public Overrides Function Clone() As DataSet
			Dim dsSafeGrd As dsSafeGrd = CType(MyBase.Clone(), dsSafeGrd)
			dsSafeGrd.InitVars()
			dsSafeGrd.SchemaSerializationMode = Me.SchemaSerializationMode
			Return dsSafeGrd
		End Function
		...
		Friend Sub InitVars(initTable As Boolean)
			Me.tableDataTable1 = CType(MyBase.Tables("DataTable1"), dsSafeGrd.DataTable1DataTable)
			If initTable Then
		...
		Private Sub InitClass()
			Me.DataSetName = "dsSafeGrd"
			Me.Prefix = ""
			Me.[Namespace] = "http://tempuri.org/dsSafeGrd.xsd"
			Me.EnforceConstraints = True
			Me.SchemaSerializationMode = SchemaSerializationMode.IncludeSchema
			Me.tableDataTable1 = New dsSafeGrd.DataTable1DataTable()
		...
		Public Shared Function GetTypedDataSetSchema(xs As XmlSchemaSet) As XmlSchemaComplexType
			Dim dsSafeGrd As dsSafeGrd = New dsSafeGrd()
			...
			xmlSchemaAny.[Namespace] = dsSafeGrd.[Namespace]
			xmlSchemaSequence.Items.Add(xmlSchemaAny)
			xmlSchemaComplexType.Particle = xmlSchemaSequence
			Dim schemaSerializable As XmlSchema = dsSafeGrd.GetSchemaSerializable()
		...
		Public Delegate Sub DataTable1RowChangeEventHandler(sender As Object, e As dsSafeGrd.DataTable1RowChangeEvent)

		<XmlSchemaProvider("GetTypedTableSchema"), Serializable()>
		Public Class DataTable1DataTable
			Inherits TypedTableBase(Of dsSafeGrd.DataTable1Row)
			...
			Private columnavg As DataColumn
			...
			End Sub
			Private Shared Sub __ENCAddToList(value As Object)
				Dim _ENCList As List(Of WeakReference) = dsSafeGrd.DataTable1DataTable.__ENCList
				Dim flag As Boolean = False
				Try
					Monitor.Enter(_ENCList, flag)
					Dim flag2 As Boolean = dsSafeGrd.DataTable1DataTable.__ENCList.Count = dsSafeGrd.DataTable1DataTable.__ENCList.Capacity
					If flag2 Then
						Dim num As Integer = 0
						Dim num2 As Integer = 0
						Dim num3 As Integer = dsSafeGrd.DataTable1DataTable.__ENCList.Count - 1
						Dim num4 As Integer = num2
						While True
							Dim num5 As Integer = num4
							Dim num6 As Integer = num3
							If num5 > num6 Then
								Exit While
							End If
							Dim weakReference As WeakReference = dsSafeGrd.DataTable1DataTable.__ENCList(num4)
							flag2 = weakReference.IsAlive
							If flag2 Then
								Dim flag3 As Boolean = num4 <> num
								If flag3 Then
									dsSafeGrd.DataTable1DataTable.__ENCList(num) = dsSafeGrd.DataTable1DataTable.__ENCList(num4)
								End If
								num += 1
							End If
							num4 += 1
						End While
						dsSafeGrd.DataTable1DataTable.__ENCList.RemoveRange(num, dsSafeGrd.DataTable1DataTable.__ENCList.Count - num)
						dsSafeGrd.DataTable1DataTable.__ENCList.Capacity = dsSafeGrd.DataTable1DataTable.__ENCList.Count
					End If
					dsSafeGrd.DataTable1DataTable.__ENCList.Add(New WeakReference(RuntimeHelpers.GetObjectValue(value)))
				Finally
					Dim flag3 As Boolean = flag
					If flag3 Then
						Monitor.[Exit](_ENCList)
			...
			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Sub New()
				dsSafeGrd.DataTable1DataTable.__ENCAddToList(Me)
			...
			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public ReadOnly Property avgColumn As DataColumn
				Get
					Return Me.columnavg
				End Get
			End Property
			...
			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public ReadOnly Default Property Item(index As Integer) As dsSafeGrd.DataTable1Row
				Get
					Return CType(Me.Rows(index), dsSafeGrd.DataTable1Row)
				End Get
			End Property

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Event DataTable1RowChanging As dsSafeGrd.DataTable1RowChangeEventHandler

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Event DataTable1RowChanged As dsSafeGrd.DataTable1RowChangeEventHandler

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Event DataTable1RowDeleting As dsSafeGrd.DataTable1RowChangeEventHandler

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Event DataTable1RowDeleted As dsSafeGrd.DataTable1RowChangeEventHandler

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
			Public Sub AddDataTable1Row(row As dsSafeGrd.DataTable1Row)
				Me.Rows.Add(row)
			End Sub

			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Function AddDataTable1Row(no As String, name As String, val As String, avg As String, total As String, DataColumn11 As String, DataColumn1 As String) As dsSafeGrd.DataTable1Row
				Dim dataTable1Row As dsSafeGrd.DataTable1Row = CType(Me.NewRow(), dsSafeGrd.DataTable1Row)
				Dim itemArray As Object() = New Object() { no, name, val, avg, total, DataColumn11, DataColumn1 }
			...
			Public Overrides Function Clone() As DataTable
				Dim dataTable1DataTable As dsSafeGrd.DataTable1DataTable = CType(MyBase.Clone(), dsSafeGrd.DataTable1DataTable)
			...
			Protected Overrides Function CreateInstance() As DataTable
				Return New dsSafeGrd.DataTable1DataTable()
			End Function
			...
			Private Sub InitClass()
				...
				Me.columnavg = New DataColumn("avg", GetType(String), Nothing, MappingType.Element)
				...
				Me.columnavg.Caption = "DataColumn1"
				Me.columntotal.Caption = "DataColumn1"
				Me.columnDataColumn11.Caption = "DataColumn1"
			End Sub

			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Function NewDataTable1Row() As dsSafeGrd.DataTable1Row
				Return CType(Me.NewRow(), dsSafeGrd.DataTable1Row)
			End Function

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
			Protected Overrides Function NewRowFromBuilder(builder As DataRowBuilder) As DataRow
				Return New dsSafeGrd.DataTable1Row(builder)
			End Function

			<GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0"), DebuggerNonUserCode()>
			Protected Overrides Function GetRowType() As Type
				Return GetType(dsSafeGrd.DataTable1Row)
			End Function
			...
			Protected Overrides Sub OnRowChanged(e As DataRowChangeEventArgs)
				...
				If flag Then
					Dim dataTable1RowChangedEvent As dsSafeGrd.DataTable1RowChangeEventHandler = Me.DataTable1RowChangedEvent
					flag = (dataTable1RowChangedEvent IsNot Nothing)
					If flag Then
						dataTable1RowChangedEvent(Me, New dsSafeGrd.DataTable1RowChangeEvent(CType(e.Row, dsSafeGrd.DataTable1Row), e.Action))
					End If
				End If
			End Sub
			...
			Protected Overrides Sub OnRowChanging(e As DataRowChangeEventArgs)
				...
				If flag Then
					Dim dataTable1RowChangingEvent As dsSafeGrd.DataTable1RowChangeEventHandler = Me.DataTable1RowChangingEvent
					flag = (dataTable1RowChangingEvent IsNot Nothing)
					If flag Then
						dataTable1RowChangingEvent(Me, New dsSafeGrd.DataTable1RowChangeEvent(CType(e.Row, dsSafeGrd.DataTable1Row), e.Action))
					End If
				End If
			End Sub
			...
			Protected Overrides Sub OnRowDeleted(e As DataRowChangeEventArgs)
				...
				If flag Then
					Dim dataTable1RowDeletedEvent As dsSafeGrd.DataTable1RowChangeEventHandler = Me.DataTable1RowDeletedEvent
					flag = (dataTable1RowDeletedEvent IsNot Nothing)
					If flag Then
						dataTable1RowDeletedEvent(Me, New dsSafeGrd.DataTable1RowChangeEvent(CType(e.Row, dsSafeGrd.DataTable1Row), e.Action))
					End If
				End If
			End Sub
			...
			Protected Overrides Sub OnRowDeleting(e As DataRowChangeEventArgs)
				...
				If flag Then
					Dim dataTable1RowDeletingEvent As dsSafeGrd.DataTable1RowChangeEventHandler = Me.DataTable1RowDeletingEvent
					flag = (dataTable1RowDeletingEvent IsNot Nothing)
					If flag Then
						dataTable1RowDeletingEvent(Me, New dsSafeGrd.DataTable1RowChangeEvent(CType(e.Row, dsSafeGrd.DataTable1Row), e.Action))
					End If
				End If
			End Sub

			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Sub RemoveDataTable1Row(row As dsSafeGrd.DataTable1Row)
				Me.Rows.Remove(row)
			End Sub
			...
			Public Shared Function GetTypedTableSchema(xs As XmlSchemaSet) As XmlSchemaComplexType
				...
				xmlSchemaAttribute.FixedValue = dsSafeGrd.[Namespace]
		...
		Public Class DataTable1Row
			...
			Friend Sub New(rb As DataRowBuilder)
				MyBase.New(rb)
				Me.tableDataTable1 = CType(Me.Table, dsSafeGrd.DataTable1DataTable)
			End Sub
			...
			Public Property avg As String
				Get
					Dim result As String
					Try
						result = Conversions.ToString(Me(Me.tableDataTable1.avgColumn))
					Catch innerException As InvalidCastException
						Throw New StrongTypingException("The value for column 'avg' in table 'DataTable1' is DBNull.", innerException)
					End Try
					Return result
				End Get
				Set(value As String)
			...
			Public Sub SetavgNull()
				Me(Me.tableDataTable1.avgColumn) = RuntimeHelpers.GetObjectValue(Convert.DBNull)
			End Sub
		...
		Public Class DataTable1RowChangeEvent
			...
			Private eventRow As dsSafeGrd.DataTable1Row

			Private eventAction As DataRowAction

			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public Sub New(row As dsSafeGrd.DataTable1Row, action As DataRowAction)
				Me.eventRow = row
				Me.eventAction = action
			End Sub

			<DebuggerNonUserCode(), GeneratedCode("System.Data.Design.TypedDataSetGenerator", "4.0.0.0")>
			Public ReadOnly Property Row As dsSafeGrd.DataTable1Row
				Get
					Return Me.eventRow
				End Get
			End Property
